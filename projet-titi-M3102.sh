...
echo "  UDP -> Network_Time_Protocol [color=red]" >> dot-titi-projet-M3102.txt #123
...
echo "  UDP -> DHCP_Client [color=violet]" >> dot-titi-projet-M3102.txt #68
...
echo "  TCP -> LDAP [color=lawngreen]" >> dot-titi-projet-M3102.txt #389
...
push-dot () {
	...
	for z in $carte
	do
	        echo "  "\"$src"\" -> "\"$z"\" [label="\"$i"\"] [color="\"$3"\"]" >> dot-titi-projet-M3102.txt 
	        src=$z
	done
